...
export default function BlogCard({ post }: BlogCardProps) {
  ...
  return (
    <Link href={`/post/${post.id}`}>
      <article className="bg-white rounded-xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 hover:border-blue-500 transition-all cursor-pointer overflow-hidden group">
        ...
        <div className="p-6">
          <div className="flex justify-between items-start mb-4">
            <h2 className="text-2xl font-bold text-navy-900 group-hover:text-blue-600 transition-colors">{post.title}</h2>
          ...
          <p className="text-navy-800 mb-4">{post.excerpt}</p>
          ...
          <button className="w-full mt-6 mb-4 px-6 py-3 bg-gradient-to-r from-gray-900 to-black text-white rounded-full hover:opacity-90 transition-all duration-300 flex items-center justify-center gap-2 group">
            Read More
